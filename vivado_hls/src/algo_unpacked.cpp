...
////////////////////////////////////////////////////////////
	...
	ap_uint<23> so_in_jet_cr[256];
	ap_uint<23> so_out_jet_cr[256];

	ap_uint<23> so_in_jet_fwd[256];
	ap_uint<23> so_out_jet_fwd[256];

	ap_uint<23> so_in_jet_boosted[256];
	ap_uint<23> so_out_jet_boosted[256];

	ap_uint<23> so_in_tau_noniso[256];
	ap_uint<23> so_out_tau_noniso[256];

	ap_uint<23> so_in_tau_iso[256];
	ap_uint<23> so_out_tau_iso[256];

	ap_uint<23> so_in_eg_noniso[256];
	ap_uint<23> so_out_eg_noniso[256];

	ap_uint<23> so_in_eg_iso[256];
	ap_uint<23> so_out_eg_iso[256];
...
#pragma HLS ARRAY_PARTITION variable=et_jet_cntr complete dim=0
#pragma HLS ARRAY_PARTITION variable=et_jet_fwd complete dim=0
...
#pragma HLS UNROLL
			...
			if (reg <= 6)
			{
				...
				so_in_jet_fwd[idx_out].range(bit4, bit3) = idx_in;
				so_in_jet_fwd[idx_out].range(bit6, bit5) = calo_regions[idx_in].rloc_phi;
				so_in_jet_fwd[idx_out].range(bit8, bit7) = calo_regions[idx_in].rloc_eta;
			}
			else if (reg >= 19)
			{
			...
			else
			{
				...
				so_in_jet_cr[idx_out].range(bit4, bit3) = idx_in;
				so_in_jet_cr[idx_out].range(bit6, bit5) = calo_regions[idx_in].rloc_phi;
				so_in_jet_cr[idx_out].range(bit8, bit7) = calo_regions[idx_in].rloc_eta;
			}
		}
	}
...
#pragma HLS UNROLL
		...
		so_in_eg_noniso[idx].range(bit4, bit3) = idx;
		so_in_eg_noniso[idx].range(bit6, bit5) = centr_region_pu_sub[idx].rloc_phi;
		so_in_eg_noniso[idx].range(bit8, bit7) = centr_region_pu_sub[idx].rloc_eta;

		so_in_eg_iso[idx].range(bit2, bit1) = Iso_egamma_et[idx];
		so_in_eg_iso[idx].range(bit4, bit3) = idx;
		so_in_eg_iso[idx].range(bit6, bit5) = centr_region_pu_sub[idx].rloc_phi;
		so_in_eg_iso[idx].range(bit8, bit7) = centr_region_pu_sub[idx].rloc_eta;

		so_in_tau_noniso[idx].range(bit2, bit1) = nonIso_tau_et[idx];
		so_in_tau_noniso[idx].range(bit4, bit3) = idx;
		so_in_tau_noniso[idx].range(bit6, bit5) = centr_region_pu_sub[idx].rloc_phi;
		so_in_tau_noniso[idx].range(bit8, bit7) = centr_region_pu_sub[idx].rloc_eta;

		so_in_tau_iso[idx].range(bit2, bit1) = Iso_tau_et[idx];
		so_in_tau_iso[idx].range(bit4, bit3) = idx;
		so_in_tau_iso[idx].range(bit6, bit5) = centr_region_pu_sub[idx].rloc_phi;
		so_in_tau_iso[idx].range(bit8, bit7) = centr_region_pu_sub[idx].rloc_eta;

		so_in_jet_boosted[idx].range(bit2, bit1) = et_jet_boosted[idx];
		so_in_jet_boosted[idx].range(bit4, bit3) = idx;
		so_in_jet_boosted[idx].range(bit6, bit5) = centr_region_pu_sub[idx].rloc_phi;
		so_in_jet_boosted[idx].range(bit8, bit7) = centr_region_pu_sub[idx].rloc_eta;
	}
...
////////////////////////////////////////////////////////////
	...
	bitonicSort256(so_in_jet_cr, so_out_jet_cr);
	bitonicSort256(so_in_jet_fwd, so_out_jet_fwd);
	bitonicSort256(so_in_jet_boosted, so_out_jet_boosted);

	bitonicSort256(so_in_eg_noniso, so_out_eg_noniso);
	bitonicSort256(so_in_eg_iso, so_out_eg_iso);

	bitonicSort256(so_in_tau_noniso, so_out_tau_noniso);
	bitonicSort256(so_in_tau_iso, so_out_tau_iso);

////////////////////////////////////////////////////////////
	// Assign the top 8 candidates to algorithnm outputs
	int olink;
	for (int idx = 255; idx > 247; idx--)
	{
#pragma HLS UNROLL
		ap_uint<1> side;
		ap_uint<9> idx_srt;
		...
		{ // Boosted jets
			...
			int bLo13 = bHi12 + 1;
			int bHi13 = bLo13 + 2;
			tmp_link_out[idx_out].range(bHi13, bLo13) = rPhi_jet_boosted[idx_srt].to_ulong();

			int bLo14 = bHi13 + 1;
			int bHi14 = bLo14 + 2;
			tmp_link_out[idx_out].range(bHi14, bLo14) = rEta_jet_boosted[idx_srt].to_ulong();
